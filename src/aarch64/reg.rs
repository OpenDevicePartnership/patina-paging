...
pub fn replace_live_xlat_entry_v2 (entry_ptr: u64, val: u64, addr: u64) {
    unsafe {
        let mut daif_val: u64;
        asm!(
            "mrs   {}, daif",
            "msr   daifset, #0xf",
            "isb",
        ...
        log::info!("Replacing entry at {:x} with {:x}", entry_ptr, val);
        ...
        log::info!("Writing {:x} to {:x}", val, entry_ptr);
